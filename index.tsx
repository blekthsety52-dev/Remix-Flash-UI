...
function App() {
  ...
  const [drawerState, setDrawerState] = useState<{
      isOpen: boolean;
      mode: 'code' | 'variations' | 'history' | 'settings' | null;
      title: string;
      data: any; 
  ...
  // Theme management
  useEffect(() => {
      const savedTheme = localStorage.getItem('flash_ui_theme') as 'light' | 'dark' | null;
      if (savedTheme) {
          setTheme(savedTheme);
...
\`{ "name": "Persona Name", "html": "..." }\`
  ...
  const handleDownloadCode = () => {
      if (drawerState.mode === 'code' && drawerState.data) {
          ...
          a.href = url;
          a.download = `component-${generateId()}.html`;
          a.click();
  ...
  const handleExportReact = () => {
      ...
      const componentName = `FlashComponent_${generateId().replace(/-/g, '_')}`;
      
      // Extract CSS from the HTML
      const styleRegex = /<style[^>]*>([\s\S]*?)<\/style>/gi;
      let css = '';
      let match;
      while ((match = styleRegex.exec(artifact.html)) !== null) {
          css += match[1] + '\n';
      }
      
      // Remove styles from the HTML to avoid duplication
      const htmlWithoutStyles = artifact.html.replace(/<style[^>]*>[\s\S]*?<\/style>/gi, '').trim();
      
      // Escape backticks and dollar signs for the template literal
      const escapedCss = css.replace(/`/g, '\\`').replace(/\$/g, '\\$');
      const escapedHtml = htmlWithoutStyles.replace(/`/g, '\\`').replace(/\$/g, '\\$');
...
`.trim();
      ...
      const blob = new Blob([reactTemplate], { type: 'text/typescript' });
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
  ...
  const handleShowHistory = () => {
      setDrawerState({ isOpen: true, mode: 'history', title: 'Session History', data: null });
  };
  ...
  const handleDeleteSession = (id: string, e: React.MouseEvent) => {
      ...
      setSessions(prev => {
          ...
          if (filtered.length === 0) {
              ...
              localStorage.removeItem('flash_ui_sessions');
          } else if (currentSessionIndex >= filtered.length) {
              setCurrentSessionIndex(filtered.length - 1);
          }
          return filtered;
      });
  };
  ...
  const handleSendMessage = useCallback(async (manualPrompt?: string) => {
    ...
    try {
        ...
        if (!currentApiKey) throw new Error("API_KEY is not configured. Please select an API key.");
...
Return ONLY a raw JSON array of 3 *NEW*, creative names for these directions (e.g. ["Tactile Risograph Press", "Kinetic Silhouette Balance", "Primary Pigment Gridwork"]).
        ...
        const styleResponse = await callWithRetry(() => ai.models.generateContent({
            model: 'gemini-3-flash-preview',
...
Return ONLY RAW HTML. No markdown fences.
          `.trim();
                ...
                if (errorMessage.includes('429')) {
                    errorMessage = "Quota exceeded (429). Try again in a moment or use your own API key.";
                } else if (errorMessage.includes('403')) {
                    errorMessage = "Permission denied (403). Ensure your API key has access to Gemini 3 Flash.";
                }
                ...
                setSessions(prev => prev.map(sess => 
                    sess.id === sessionId ? {
                        ...sess,
                        artifacts: sess.artifacts.map(art => 
                            art.id === artifact.id ? { ...art, html: `<div style="color: #ff6b6b; padding: 20px;">Error: ${errorMessage}</div>`, status: 'error' } : art
                        )
                    } : sess
                ));
            }
  ...
  const renderHeaderActions = () => {
      if (drawerState.mode === 'code') {
          return (
              <>
                  <button className="header-action-button" onClick={handleCopyCode}>
                      <CopyIcon /> Copy
                  </button>
                  <button className="header-action-button" onClick={handleDownloadCode}>
                      <DownloadIcon /> Download
                  </button>
              </>
          );
      }
      if (drawerState.mode === 'history') {
          return (
              <button className="header-action-button" onClick={handleClearHistory} style={{ color: '#ff4d4d' }}>
                  <TrashIcon /> Clear All
              </button>
          );
      }
    ...
    <>
            ...
            {drawerState.mode === 'history' && (
                <div className="history-list">
                    {sessions.map((s, i) => (
                        <div 
                            key={s.id} 
                            className={`history-item ${i === currentSessionIndex ? 'active' : ''}`}
                            onClick={() => {
                        ...
                        >
                            ...
                            <button className="delete-history-btn" onClick={(e) => handleDeleteSession(s.id, e)}>
                                <TrashIcon />
                            </button>
                        </div>
                    )).reverse()}
        ...
        <div className="immersive-app">
            ...
            <DottedGlowBackground 
                ...
                glowColor={theme === 'light' ? "rgba(0, 0, 0, 0.2)" : "rgba(255, 255, 255, 0.15)"} 
                speedScale={0.5} 
            />
                 ...
                 <div className={`empty-state ${hasStarted ? 'fade-out' : ''}`}>
                     <div className="empty-content">
                         <h1 onClick={() => {
                             setCurrentSessionIndex(-1);
                             setFocusedArtifactIndex(null);
                         }} style={{ cursor: 'pointer' }}>Flash UI</h1>
                         ...
                         <div style={{ display: 'flex', gap: '12px' }}>
                            ...
                            {sessions.length > 0 && (
                                <button className="surprise-button" onClick={handleShowHistory} style={{ background: 'rgba(255,255,255,0.05)' }}>
                                    <HistoryIcon /> History
                                </button>
                            )}
                         </div>
            ...
            <div className={`action-bar ${currentSessionIndex !== -1 ? 'visible' : ''}`}>
                 ...
                 <div className="action-buttons">
                    ...
                    {focusedArtifactIndex !== null && (
                        <>
                            ...
                            <button onClick={handleExportReact}>
                                <ReactIcon /> Export React
                            </button>
                        </>
                    )}
                 </div>
